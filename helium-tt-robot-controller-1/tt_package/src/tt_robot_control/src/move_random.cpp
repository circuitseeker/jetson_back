...
int main(int argc, char **argv)
{
  ...
  {
    RCLCPP_ERROR(node->get_logger(), "Expected 5 joints, but found %zu.", joint_names.size());
    return 1;
  }
  RCLCPP_INFO(node->get_logger(), "Found %zu joints.", joint_names.size());

  // Setup random generators for each joint with limits from URDF
  std::random_device rd;
  std::mt19937 gen(rd());
  
  // Joint limits from your URDF:
  // jlink1: prismatic, -0.5 to 0.5
  // jlink2: revolute, -3.14 to 3.14
  // jlink3: revolute, -1.57 to 1.57
  // jlink4: revolute, -1.57 to 1.57
  // jlink5: revolute, -1.57 to 1.57
  std::uniform_real_distribution<double> dis_joint1(-0.5, 0.5);      // Joint 1: prismatic
  std::uniform_real_distribution<double> dis_joint2(-M_PI, M_PI);   // Joint 2: full rotation
  std::uniform_real_distribution<double> dis_joint3(-1.57, 1.57);   // Joint 3: ±90°
  std::uniform_real_distribution<double> dis_joint4(-1.57, 1.57);   // Joint 4: ±90°
  std::uniform_real_distribution<double> dis_joint5(-1.57, 1.57);   // Joint 5: ±90°
  ...
  {
    // Generate a joint target vector for 5 joints
    std::vector<double> joint_target(5);
